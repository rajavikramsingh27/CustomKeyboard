...
//            self.returnButton.setTitle(retTitle, for: .normal)
            self.returnButton.setTitle("Send", for: .normal)
        }
    }
...
//        self.returnButton = accessoryButtons(title: "return", img: nil, tag: 7)
        self.returnButton = accessoryButtons(title: "Send", img: nil, tag: 7)
...
//        let fourthRowSV = UIStackView(arrangedSubviews: [self.numericButton,self.nextKeyboardButton,self.rupeSoButton,spaceKey,self.returnButton])
        let fourthRowSV = UIStackView(arrangedSubviews: [self.numericButton,spaceKey,self.returnButton])
        
        fourthRowSV.distribution = .fillEqually//.fillProportionally
    ...
    func addRowsOnKeyboard(kbKeys: [String]) -> UIView {
        ...
        let keysView = UIView()
        keysView.backgroundColor = .clear // key board background color
...
//             self.textDocumentProxy.insertText("\n")
            self.view.window?.rootViewController?.view.resignFirstResponder()
        }
